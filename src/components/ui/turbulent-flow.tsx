...
type Props = {
  className?: string;
  /** Darken canvas so page content stays readable */
  withScrim?: boolean;
};

/**
 * Full-viewport turbulent gradient WebGL background (Three.js + GSAP).
 */
export default function TurbulentFlowBackground({ className, withScrim = false }: Props) {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (mount && canvas.parentNode === mount) {
  ...
  return (
    ...
    >
      ...
      {withScrim ? (
        <div className="absolute inset-0 z-[1] bg-[#13131b]/55" />
      ) : null}
    </div>
  );
}
