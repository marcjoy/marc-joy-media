...
/**
 * Full-viewport WebGL neural / vortex field (pointer + scroll reactive).
 * Background only — no layout or typography; use on pages like Music.
 */
export default function InteractiveNeuralVortexBackground({
  ...
  useEffect(() => {
    ...
    if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
      console.error('Neural vortex program link error:', gl.getProgramInfoLog(program));
  ...
  return (
    ...
    >
      ...
      />
      {withScrim ? (
        <div className="absolute inset-0 z-[1] bg-[#0a0520]/55" aria-hidden />
      ) : null}
    </div>
  );
}
